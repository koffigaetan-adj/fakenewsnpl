...
if page == "À Propos":
    st.markdown(f"""
    <div style='background:linear-gradient(135deg,#0c0c20 0%,#131330 50%,#0c0c20 100%);
                border:1px solid #1e1e3a;border-radius:20px;padding:40px 44px;margin-bottom:28px;
      ...
      <span class='badge' style='margin-bottom:16px;'>{ico_html('info',12)} Contexte Académique</span>
      <h1 style='font-size:38px!important;margin:0 0 10px;background:linear-gradient(135deg,#fff 40%,#818cf8 100%);
                 -webkit-background-clip:text;-webkit-text-fill-color:transparent;'>
        TruthScope — À Propos
      </h1>
      <p style='font-size:15px!important;color:#94a3b8!important;max-width:700px;line-height:1.8;'>
        Projet de fin de Spécialisation — <b style='color:#818cf8;'>MSc Business & Technology Manager · Spé 3</b><br>
        Epitech Digital School &bull; Promotion DIGI_2026_PA
      </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ---- Bloc contexte académique ----
    col1, col2 = st.columns([3, 2])
    with col1:
        st.markdown(f"""
        <div class='card'>
          <span class='badge'>{ico_html('globe', 12)} Contexte du Projet</span>
          <h3>Validation Spé 3 — MSc Business & Technology Manager</h3>
          <p>
            Ce projet constitue le livrable principal de la <b style='color:#818cf8;'>Spécialisation 3</b>
            du <b style='color:#818cf8;'>MSc Business & Technology Manager</b> dispensé à Epitech Digital School.
            Il s'inscrit dans une démarche concrète d'application des compétences en <b style='color:#818cf8;'>
            traitement du langage naturel (NLP)</b> et en <b style='color:#818cf8;'>Machine Learning</b>
            à une problématique d'actualité majeure : la détection automatique de <b style='color:#ef4444;'>
            fake news</b> dans le discours politique.
          </p>
          <p>
            Dans un contexte de montée en puissance de la désinformation sur les réseaux sociaux et
            dans les médias, la capacité à identifier automatiquement des contenus trompeurs représente
            un enjeu stratégique pour les organisations, les médias, et les décideurs. Ce projet répond
    ...
    st.markdown(f"""
    <div class='section-title'>
      {ico_html('cpu', 22, '#6366f1')}
      <div><h2 style='margin:0;'>Démarche Scientifique & Pipeline</h2></div>
    </div>
    """, unsafe_allow_html=True)

    pipeline_steps = [
        ("01", "bar-chart", "Exploration (EDA)", "#6366f1",
         "Analyse du LIAR Dataset : distribution des 6 labels, top speakers, partis politiques, "
         "longueur des déclarations, sujets traités et métadonnées de crédibilité. "
         "Identification des biais et contraintes du corpus (12 836 déclarations PolitiFact)."),
        ("02", "settings", "Preprocessing", "#8b5cf6",
         "Pipeline de nettoyage textuel (lowercase, suppression ponctuation, stop-words NLTK). "
         "Vectorisation TF-IDF bigrammes (max 20 000 features). Encodage des labels en binaire "
         "(fake/real) et multi-classe (3 classes). Ingénierie de features métadonnées speaker."),
        ("03", "cpu", "Modèles Classiques", "#06b6d4",
         "Entraînement de 3 classifieurs : Logistic Regression, Random Forest, XGBoost. "
         "Évaluation sur le jeu de test : accuracy, F1 macro, matrice de confusion. "
    ...
    st.markdown(f"""
    <div class='section-title'>
      {ico_html('trending-up', 22, '#22c55e')}
      <div><h2 style='margin:0;'>Résultats Clés & Conclusions</h2></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""
        <div class='card'>
          <span class='badge badge-green'>Performances In-Domain</span>
          <h3>Meilleurs résultats — Test Set LIAR</h3>
          <p>• <b style='color:#22c55e;'>LR Binaire</b> : <b style='color:#fff;'>78.7% F1 macro</b> — Champion toutes catégories</p>
          <p>• <b style='color:#818cf8;'>RF Binaire</b> : 78.4% F1 macro</p>
          <p>• <b style='color:#06b6d4;'>XGBoost Binaire</b> : 77.2% F1 macro</p>
          <p>• <b style='color:#fbbf24;'>LR 3-classes</b> : 54.4% F1 macro (beaucoup plus difficile)</p>
          <p>→ La classification binaire (fake vs real) est nettement plus robuste que la multi-classe.</p>
        </div>
        """, unsafe_allow_html=True)
    with col2:
        st.markdown("""
        <div class='card'>
          <span class='badge badge-amber'>Enseignements OOD</span>
          <h3>Généralisation — WELFake</h3>
          <p>• Les modèles <b style='color:#ef4444;'>avec métadonnées speaker</b> s'effondrent en OOD (−45 pp)</p>
          <p>• Les modèles <b style='color:#22c55e;'>TF-IDF pur</b> se généralisent mieux (robustesse domaine)</p>
          <p>• Le <b style='color:#818cf8;'>credibility_score</b> est inutilisable sur WELFake (pas de profil speaker)</p>
          <p>• Les bigrammes capturent des patterns rhétoriques transférables entre domaines</p>
          <p>→ Un modèle robuste ne doit pas sur-apprendre les métadonnées contextuelles.</p>
        </div>
        """, unsafe_allow_html=True)

    st.markdown(f"""
    <div class='alert-info'>
      <p>{ico_html('info', 14, '#67e8f9')} &nbsp;
        <b>Note académique</b> — Ce dashboard est le livrable principal du projet de Spé 3 Master of Science Business & Technology Manager.
        Il implémente l'intégralité du pipeline, de la donnée brute à la démonstration live,
        et constitue un outil de <b>transparence algorithmique</b> permettant aux non-spécialistes
        de comprendre les choix de modélisation et leurs implications éthiques dans le contexte
